    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::TopBottomPanel::bottom("bottom_panel").show(ctx, |ui| {
            ui.horizontal(|ui| {
                ...
                if let Some(ref chr) = self.selected_chromosome {
                    ui.label(format!("Chromosome: {} | Position: {}-{}",
            ...
            if let Some(ref genome) = self.genome {
                egui::ScrollArea::vertical().show(ui, |ui| {
                    for (chr_name, chr_data) in &genome.chromosomes {
                        ...
                        if ui.selectable_label(is_selected, format!("{} ({} bp)", chr_name, chr_data.length))
                            .clicked() {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            if response.dragged() {
                ...
                let pixels_per_base = available_size.x / (self.viewport.end - self.viewport.start) as f32;
            ...
            // Render genome
            if let (Some(ref genome), Some(ref chr_name)) = (&self.genome, &self.selected_chromosome) {
